...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = (random.randrange(SNAKE_SIZE, SIZE[0] - SNAKE_SIZE), \
                              random.randrange(SNAKE_SIZE, SIZE[0] - SNAKE_SIZE))
    ...
    def init_food(self):
        ...
        self.available_food_positions = set(zip(*np.where(self.world == 0))) - set(snake.blocks)
        if not self.custom:
            # Choose a random position from available
            chosen_position = random.choice(list(self.available_food_positions))
        else:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if self.world[new_snake_head[0]][new_snake_head[1]] == 255:
